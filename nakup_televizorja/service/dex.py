...
class DEXFunction:
    ...
    def evaluate_QQ(self, **input):
        ...
        for attr in self.attr_list:
            ...
            if (
                (not isinstance(input[attr.name], str))
                and "*" in input[attr.name].astype(str)
            ) or (isinstance(input[attr.name], str) and input[attr.name] == "*"):
...
class DEXModel:
    ...
    def get_input_attributes_with_descriptions(self):
        attributes_with_description = {}
        for value in self.attributes.values():
            if value.level == 1:
                attributes_with_description[value.name] = {
                    'description': value.description,
                    'choices': list(value.scale.scalevalue.keys())
                }
        return attributes_with_description
